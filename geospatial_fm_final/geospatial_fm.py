...
class ConvTransformerTokensToEmbeddingNeck(nn.Module):
    ...
    def __init__(
        ...
        #   out_ch = self.output_embed_dim if i != 0 else self.output_embed_dim
        #   in_ch = self.embed_dim if i == 0 else self.embed_dim * 2
        #   conv = nn.Sequential(
        #       nn.Conv2d(
        #           in_ch,
        #           out_ch,
        ...
        #       nn.GELU(),
        #   )
        #   # self.unet_convs.append(conv)
        #   setattr(self, f'unet_l{i}', conv)
    ...
    def forward(self, xs):
        ...
        # x = self.unet_l1(xs[0])
        # print('first', x.shape)
        # for i, other_x in enumerate(xs[1:]):
        #   conv_layer = getattr(self, f'unet_l{i+1}')
        #   other_x = conv_layer(other_x)
        #   print('other', other_x.shape)
        #   x = torch.cat([x, other_x], dim=1)

        # x = self.unet_l0(xs[-1])
        # xs = xs[:-1]
        # print('first', x.shape)
        # for i, other_x in enumerate(reversed(xs)):
        #   x = torch.cat([x, other_x], dim=1)
